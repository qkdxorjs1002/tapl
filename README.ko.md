...
`taplctl`은 한 번 설치합니다. 각 Codex workspace는 `.tapl/workspace.toml`
앵커와 `.tapl/tapl.db`를 가집니다. 첫 hook event에서 상위 앵커를 찾지 못하면
payload 작업 폴더를 명시적으로 초기화합니다. 이후 하위 Git repository에서
명령을 실행해도 별도 history DB를 만들지 않고 해당 workspace 앵커를 재사용합니다.
...
중첩 repository를 의도적으로 독립 관리할 때만 그 위치를 별도 workspace로
명시적으로 초기화합니다.
