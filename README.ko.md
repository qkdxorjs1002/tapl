...
세션의 첫 구체적 요청에서는 사용 가능한 전체 위임 모델 목록도 같은 호출에 넣습니다.
...
전달할 수 있습니다. 같은 revision이면 변경 없는 필드만 생략하며, 상태 요약·다음 행동과 모델
...
`get_next`를 다시 호출하지 않습니다. 오류·응답 잘림, 정책·컨텍스트 유실, 모델 목록 변경,
