...
<body>
  ...
  <script type="module" src="${scriptUri}"></script>
    ...
    async openArchiveEventsById(archiveId) {
        if (this.currentView.type === 'archive' && this.currentView.archive.id === archiveId) {
            await this.navigate({ type: 'archiveEvents', archive: this.currentView.archive, detail: this.currentView.detail }, { pushHistory: true, reveal: false });
            return;
        }
        const detail = await safeArchiveDetail(archiveId);
        if (detail.ok) {
            await this.navigate({ type: 'archiveEvents', archive: detail.value.archive, detail: detail.value }, { pushHistory: true, reveal: false });
            return;
        }
        const archives = await safeArchives();
        if (!archives.ok) {
...
function renderArchiveEventsView(archive, detail) {
    const archiveMeta = detail?.archive ?? archive;
...
function renderActiveBatchesPanel(batches) {
    if (!batches.length) {
        return '';
    }
...
async function enrichTaskDetail(detail) {
    if (detail.kind !== 'task') {
        return detail;
    }
    const status = await safeTapl(['status', '--json', '--full']);
