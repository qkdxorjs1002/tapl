...
class WorkflowWebviewManager {
    ...
    async renderCurrentView(webview) {
        switch (this.currentView.type) {
            case 'document':
                return renderPage(webview, await renderDocumentView(this.currentView.uri, this.currentView.title), `document:${relativePath(this.currentView.uri)}`);
            case 'archive':
                return renderPage(webview, await renderArchiveView(this.currentView.uri, this.currentView.title), `archive:${relativePath(this.currentView.uri)}`);
            case 'overview':
            default:
...
async function loadArchiveListItems(archiveRootUri, folderNames) {
...
function extractArchiveSummary(markdown, folderName) {
    if (!markdown) {
        return folderName;
    }
...
function firstUsefulSummaryLine(markdown) {
    if (!markdown) {
        return undefined;
    }
...
function renderPage(webview, body, viewStateKey) {
